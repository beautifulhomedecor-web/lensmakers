          ...
          return (
            <button
              ...
              style={{
                ...
                background: isActive ? 'var(--color-accent-primary)' : 'rgba(255,240,224,0.65)',
            ...
            >
              <span>{tab.label}</span>
              {tab.badge && (
                <span style={{ background: isActive ? 'rgba(0,0,0,0.25)' : '#013E37', color: 'var(--color-text-primary)', padding: '2px 6px', borderRadius: '999px', fontSize: '9px', fontWeight: '900' }}>
                  {tab.badge}
                </span>
              )}
            </button>
        ...
        <div style={{ padding: '16px var(--screen-padding)' }}>
          ...
          <div className="flex-between mb-3">
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <h1 style={{ fontSize: '22px', fontWeight: '800', color: 'var(--color-text-primary)' }}>Lens Makers Club</h1>
            </div>
          ...
          >
            ...
            <div className="flex-between" style={{ position: 'relative', zIndex: 2 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontSize: '13px', fontWeight: '800', color: 'var(--color-text-primary)', letterSpacing: '1px' }}>LENS MAKERS</span>
              </div>
              <span style={{ fontSize: '12px', fontWeight: '900', color: 'var(--color-text-primary)', letterSpacing: '3px', textTransform: 'uppercase' }}>CLUB VIP</span>
            </div>

            {/* Center: Large Pricing Display */}
            <div style={{ position: 'relative', zIndex: 2, margin: '14px 0' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px' }}>
                <span style={{ fontSize: '46px', fontWeight: '900', color: 'var(--color-accent-primary)', letterSpacing: '-1px' }}>₹99</span>
                <span style={{ fontSize: '20px', color: 'var(--color-text-secondary)', fontWeight: '600' }}>/ month</span>
              </div>
            </div>
            ...
            <div className="flex-between" style={{ position: 'relative', zIndex: 2 }}>
              <span style={{ fontSize: '13px', fontWeight: '800', color: 'var(--color-text-primary)', background: 'rgba(1,62,55,0.15)', padding: '4px 12px', borderRadius: '999px', border: '1px solid rgba(1,62,55,0.3)' }}>
                ⚡ 25% OFF EVERYTHING
              </span>
              <span style={{ fontSize: '11px', fontWeight: '700', color: 'var(--color-text-secondary)' }}>VALID WORLDWIDE</span>
            </div>
          </div>

          {/* Headline Below Card */}
          <div style={{ textAlign: 'center', marginBottom: '28px' }}>
            <h2 style={{ fontSize: '26px', fontWeight: '900', color: 'var(--color-text-primary)', marginBottom: '8px', letterSpacing: '-0.5px' }}>
              Everything You Love, 25% Cheaper.
            </h2>
            <p style={{ fontSize: '14px', color: 'var(--color-text-secondary)', lineHeight: '1.5', maxWidth: '330px', margin: '0 auto' }}>
          ...
          <div className="glass-card-standard mb-4" style={{ padding: '8px 16px' }}>
            {[
              { icon: '🏷️', color: 'var(--color-accent-primary)', title: 'Flat 25% Off All Eyewear', desc: 'Every frame, every order — no minimum, no exceptions.' },
              { icon: '👁️', color: 'var(--color-text-primary)', title: '1 Free Online Eye Check-Up/Month', desc: 'Book your monthly vision check — completely free.' },
              { icon: '📸', color: 'var(--color-text-primary)', title: 'Priority AR Try-On Slots', desc: 'Get first access to new frames before general release.' },
              { icon: '🎁', color: 'var(--color-text-primary)', title: 'Exclusive Member-Only Offers', desc: 'Special BOGO deals, flash sales, and birthday offers.' },
              { icon: '📞', color: 'var(--color-text-primary)', title: 'Dedicated Support Line', desc: 'Skip the queue — member-only priority 24/7 support.' }
            ].map((ben, idx) => (
              <div
                key={idx}
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '15px', fontWeight: '800', color: 'var(--color-text-primary)', marginBottom: '3px' }}>{ben.title}</div>
                  <div style={{ fontSize: '12px', color: 'var(--color-text-secondary)', lineHeight: '1.4' }}>{ben.desc}</div>
                </div>
                <div style={{ color: 'var(--color-success)', fontSize: '18px', fontWeight: '900' }}>✓</div>
              </div>
            ))}
          </div>

          {/* PRICING SECTION (2 Side-by-Side Options) */}
          <h3 style={{ fontSize: '18px', fontWeight: '800', color: 'var(--color-text-primary)', marginBottom: '14px', textAlign: 'center' }}>
          ...
          <div className="grid-2 mb-3" style={{ gap: '14px' }}>
            ...
            <div className="glass-card-standard" style={{ padding: '18px 14px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', border: selectedPlan === 'monthly' ? '2px solid #A94A4A' : '1px solid rgba(255,255,255,0.15)', cursor: 'pointer' }} onClick={() => setSelectedPlan('monthly')}>
              <div>
                <div style={{ fontSize: '13px', fontWeight: '800', color: 'var(--color-text-secondary)', uppercase: 'true' }}>MONTHLY PLAN</div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', margin: '8px 0' }}>
                  <span style={{ fontSize: '28px', fontWeight: '900', color: 'var(--color-accent-primary)' }}>₹99</span>
                  <span style={{ fontSize: '13px', color: 'var(--color-text-secondary)' }}>/ month</span>
                </div>
                <div style={{ fontSize: '11px', color: 'var(--color-text-secondary)', marginBottom: '4px' }}>Billed monthly</div>
                <div style={{ fontSize: '11px', color: 'var(--color-text-primary)', fontWeight: '700' }}>✓ Cancel anytime</div>
              </div>
              <button
                type="button"
                className="btn-secondary-pill w-100 mt-3"
                style={{ height: '40px', fontSize: '12px', background: 'var(--color-accent-primary)', color: 'var(--color-bg-primary)', border: 'none' }}
            ...
            <div className="glass-card-glow-pink" style={{ padding: '18px 14px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', border: '2px solid #A94A4A', position: 'relative', cursor: 'pointer' }} onClick={() => setSelectedPlan('annual')}>
              <span style={{ position: 'absolute', top: '10px', right: '10px', background: '#013E37', color: 'var(--color-text-primary)', padding: '3px 8px', borderRadius: '999px', fontSize: '9px', fontWeight: '900', letterSpacing: '0.5px' }}>
                BEST VALUE 🔥
              </span>
              <div>
                <div style={{ fontSize: '13px', fontWeight: '800', color: 'var(--color-accent-primary)', uppercase: 'true' }}>ANNUAL PLAN</div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', margin: '8px 0' }}>
                  <span style={{ fontSize: '28px', fontWeight: '900', color: 'var(--color-accent-primary)' }}>₹799</span>
                  <span style={{ fontSize: '13px', color: 'var(--color-text-secondary)' }}>/ year</span>
                </div>
                <div style={{ fontSize: '12px', color: 'var(--color-text-primary)', fontWeight: '800', marginBottom: '2px' }}>Save ₹389 vs monthly!</div>
                <div style={{ fontSize: '11px', color: 'var(--color-text-secondary)' }}>Effective ₹66.58 / month</div>
              </div>
              <button
                type="button"
                className="btn-primary-pill w-100 mt-3"
                style={{ height: '40px', fontSize: '12px', background: 'var(--color-accent-primary)' }}
          ...
          <div style={{ textAlign: 'center', fontSize: '11px', color: 'var(--color-text-secondary)', marginBottom: '32px' }}>
          ...
          <div className="glass-card-standard mb-4" style={{ padding: '20px' }}>
            <h3 style={{ fontSize: '16px', fontWeight: '800', color: 'var(--color-text-primary)', marginBottom: '16px' }}>
              Why Members Save 5x More
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', fontSize: '13px' }}>
              {/* Table Header */}
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.2fr 1.2fr', paddingBottom: '10px', borderBottom: '1.5px solid rgba(255,255,255,0.15)', fontWeight: '800', color: 'var(--color-accent-primary)' }}>
                <span>Feature</span>
                <span>Non-Member</span>
                <span style={{ color: 'var(--color-text-primary)' }}>VIP Member</span>
              </div>
              {/* Rows */}
              {[
                ...
                >
                  <span style={{ color: 'var(--color-text-primary)', fontWeight: '700' }}>{row.feat}</span>
                  <span style={{ color: 'var(--color-text-secondary)' }}>{row.std}</span>
                  <span style={{ color: row.highlight ? '#013E37' : 'var(--color-accent-primary)', fontWeight: '900' }}>{row.vip}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <h3 style={{ fontSize: '16px', fontWeight: '800', color: 'var(--color-text-primary)', marginBottom: '14px' }}>
          ...
          <div style={{ display: 'flex', gap: '14px', overflowX: 'auto', paddingBottom: '12px', marginBottom: '32px', scrollbarWidth: 'none' }}>
            {[
              ...
              { quote: "Got 2 premium titanium frames at 25% off plus BOGO. Paid less than half of what optical shops charge.", author: "Rahul M.", since: "Member since 2024", initials: "RM", color: "#0B1F1C" },
              { quote: "Priority AR Try-on and instant delivery tracking makes Lens Makers feel like an Apple product.", author: "Vikram S.", since: "Member since 2026", initials: "VS", color: "#0B1F1C" }
            ].map((t, idx) => (
              <div key={idx} className="glass-card-standard" style={{ minWidth: '260px', padding: '18px', flexShrink: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ color: 'var(--color-text-secondary)', fontSize: '14px', marginBottom: '8px' }}>★★★★★</div>
                  <p style={{ fontSize: '13px', color: 'var(--color-text-primary)', fontStyle: 'italic', lineHeight: '1.5', marginBottom: '16px' }}>
                    "{t.quote}"
                  </p>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '18px', background: t.color, color: 'var(--color-text-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: '800' }}>
                    {t.initials}
                  </div>
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: '800', color: 'var(--color-text-primary)' }}>{t.author}</div>
                    <div style={{ fontSize: '11px', color: 'var(--color-text-secondary)' }}>{t.since}</div>
                  </div>
                </div>
              </div>
            ))}
              ...
              return (
                ...
                >
                  <div className="flex-between">
                    <span style={{ fontSize: '14px', fontWeight: '800', color: isOpen ? 'var(--color-accent-primary)' : 'var(--color-bg-primary)' }}>{faq.q}</span>
                    <span style={{ fontSize: '16px', color: 'var(--color-text-secondary)', transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 200ms ease' }}>▼</span>
                  </div>
                  {isOpen && (
                    <div style={{ fontSize: '13px', color: 'var(--color-text-secondary)', lineHeight: '1.5', marginTop: '10px', paddingTop: '10px', borderTop: '1px solid rgba(255,255,255,0.06)', animation: 'fadeIn 200ms ease' }}>
                      {faq.a}
                    </div>
                  )}
                </div>
          ...
          <div style={{ position: 'sticky', bottom: '16px', zIndex: 20 }}>
            <button
              ...
              style={{ height: '56px', fontSize: '16px', fontWeight: '900', background: 'var(--color-accent-primary)', boxShadow: '0 8px 28px 
    param($m)
      ...
      {viewMode === 'payment' && (
        <div style={{ padding: '16px var(--screen-padding)' }}>
          ...
          <div className="flex-between mb-4">
            ...
            <h1 style={{ fontSize: '18px', fontWeight: '800', color: 'var(--color-text-primary)' }}>Activate VIP Club</h1>
          ...
          <div className="glass-card-glow-pink mb-4" style={{ padding: '20px', borderLeft: '4px solid #A94A4A' }}>
            <div className="flex-between mb-1">
              <span style={{ fontSize: '11px', fontWeight: '700', color: 'var(--color-text-secondary)', uppercase: 'true' }}>SUBSCRIPTION ORDER SUMMARY</span>
            ...
            <div style={{ fontSize: '13px', color: 'var(--color-text-secondary)', margin: '4px 0 12px' }}>
              {selectedPlan === 'annual' ? 'Billed once at ₹799/year (Save ₹389!)' : 'Billed monthly at ₹99/month. Cancel anytime.'}
            </div>
            <div className="flex-between" style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '12px', fontSize: '16px', fontWeight: '800', color: 'var(--color-text-primary)' }}>
              <span>Total Amount Due:</span>
              <span style={{ fontSize: '24px', fontWeight: '900', color: 'var(--color-accent-primary)' }}>{selectedPlan === 'annual' ? '₹799' : '₹99'}</span>
            </div>
          </div>

          {/* Payment Method Selector */}
          <h3 style={{ fontSize: '16px', fontWeight: '800', color: 'var(--color-text-primary)', marginBottom: '12px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '28px' }}>
            {[
              ...
              return (
                ...
                >
                  <div>
                    <div style={{ fontSize: '14px', fontWeight: '800', color: 'var(--color-text-primary)' }}>{m.label}</div>
                    <div style={{ fontSize: '12px', color: 'var(--color-text-secondary)' }}>{m.desc}</div>
                  </div>
                  <div style={{ width: '20px', height: '20px', borderRadius: '10px', border: `2px solid ${isSel ? 'var(--color-accent-primary)' : 'var(--color-text-secondary)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {isSel && <div style={{ width: '10px', height: '10px', borderRadius: '5px', background: 'var(--color-accent-primary)' }} />}
                  </div>
                </div>
              );
            })}
      ...
      {viewMode === 'welcome' && (
        <div style={{ padding: '24px var(--screen-padding)', textAlign: 'center', position: 'relative', overflow: 'hidden', minHeight: '80vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 5 }}>
            {[...Array(24)].map((_, i) => {
              const colors = ['var(--color-accent-primary)', '#013E37', '#013E37', '#013E37', 'var(--color-text-secondary)'];
          ...
          <div
            className="vip-card-visual mb-4"
            style={{ width: '320px', height: '190px', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', animation: 'slideInDown 600ms var(--spring-bezier)', boxShadow: '0 20px 50px 
    param($m)
    $opacity = $m.Groups[1].Value
    "rgba(11,31,28,0.$opacity)"
  ' }}
          >
            <div className="flex-between">
              <span style={{ fontSize: '12px', fontWeight: '900', color: 'var(--color-text-primary)', letterSpacing: '1px' }}>LENS MAKERS CLUB</span>
              <span className="badge-pill badge-green" style={{ padding: '4px 10px', fontSize: '10px' }}>ACTIVE MEMBER ✓</span>
            </div>

            <div style={{ textAlign: 'left', margin: '10px 0' }}>
              <div style={{ fontSize: '11px', color: 'var(--color-text-secondary)', uppercase: 'true' }}>MEMBER NAME</div>
              <div style={{ fontSize: '20px', fontWeight: '900', color: 'var(--color-text-primary)', letterSpacing: '0.5px' }}>Alex Rivera</div>
            </div>

            <div className="flex-between">
              <span style={{ fontSize: '12px', fontWeight: '800', color: 'var(--color-text-primary)' }}>⚡ 25% OFF ALL ORDERS</span>
              <span style={{ fontSize: '11px', color: 'var(--color-text-secondary)' }}>ID: LM-VIP-9821</span>
            </div>
          </div>

          <h1 style={{ fontSize: '28px', fontWeight: '900', color: 'var(--color-text-primary)', marginBottom: '8px', zIndex: 10 }}>
            Welcome to Lens Makers Club! 🎉
          </h1>
          <p style={{ fontSize: '15px', color: 'var(--color-text-secondary)', maxWidth: '300px', margin: '0 auto 28px', lineHeight: '1.5', zIndex: 10 }}>
          ...
          <button
            ...
            style={{ width: '280px', height: '54px', fontSize: '16px', fontWeight: '900', background: 'var(--color-accent-primary)', zIndex: 10 }}
            onClick={() => {
      ...
      {viewMode === 'manage' && (
        <div style={{ padding: '16px var(--screen-padding)' }}>
          ...
          <div className="flex-between mb-4">
            ...
            <h1 style={{ fontSize: '18px', fontWeight: '800', color: 'var(--color-text-primary)' }}>Manage Club Membership</h1>
            <span style={{ width: '36px' }} />
          </div>
          ...
          <div className="glass-card-glow-purple mb-4" style={{ padding: '20px' }}>
            <div className="flex-between mb-2">
              <span style={{ fontSize: '12px', fontWeight: '800', color: 'var(--color-text-primary)', letterSpacing: '1px' }}>VIP PRIVILEGE STATUS</span>
              <span className={`badge-pill ${window.userIsMember ? 'badge-green' : 'badge-orange'}`}>
                {window.userIsMember ? 'ACTIVE MEMBER ✓' : 'INACTIVE ✕'}
              </span>
            </div>
            <div style={{ fontSize: '20px', fontWeight: '900', color: 'var(--color-text-primary)', marginBottom: '4px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button
                ...
                style={{ flex: 1, height: '40px', fontSize: '13px', background: 'var(--color-accent-primary)', color: 'var(--color-bg-primary)', border: 'none' }}
          ...
          <div className="glass-card-standard mb-4" style={{ padding: '18px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '13px', color: 'var(--color-text-secondary)' }}>
              <div className="flex-between"><span>🏷️ Flat 25% Off All Eyewear</span><span style={{ color: 'var(--color-text-primary)', fontWeight: '800' }}>ACTIVE ✓</span></div>
              <div className="flex-between"><span>👁️ 1 Free Monthly Eye Check-Up</span><span style={{ color: 'var(--color-text-primary)', fontWeight: '800' }}>1 SLOT READY</span></div>
              <div className="flex-between"><span>📸 Priority AR Try-On Slots</span><span style={{ color: 'var(--color-text-primary)', fontWeight: '800' }}>ACTIVE ✓</span></div>
              <div className="flex-between"><span>🎁 Buy 1 Get 1 Free Offers</span><span style={{ color: 'var(--color-text-primary)', fontWeight: '800' }}>ELIGIBLE ✓</span></div>
            </div>
          </div>
